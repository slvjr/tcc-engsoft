...
class ArmazenagemScreen extends StatefulWidget {
  @override
  _ArmazenagemPageState createState() => _ArmazenagemPageState();
}

class _ArmazenagemPageState extends State<ArmazenagemScreen> {
  bool _loading = false;
  Map _tarefa = {};
  int _fase = 0;
  ...
  Future _iniciaArmazenagem() async {
    changeLoading(true);
  ...
  Future _buscaTarefa() async {
    ...
    String erro = "";
    await api.buscaUltimaTarefaExecucaoColetor();
    final busca = await api.buscaTarefa(2);
    if (busca.isNotEmpty) {
  ...
  Future<bool> _cancelaArmazenagem() async {
    if (_fase == 0) {
