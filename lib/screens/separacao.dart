...
class SeparacaoScreen extends StatefulWidget {
  @override
  _SeparacaoPageState createState() => _SeparacaoPageState();
}

class _SeparacaoPageState extends State<SeparacaoScreen> {
  bool _loading = false;
  Map _tarefa = {};
  int _fase = 0;
  ...
  Future _buscaTarefa() async {
    ...
    String erro = "";
    await api.buscaUltimaTarefaExecucaoColetor();
    final busca = await api.buscaTarefa(3);
    if (busca.isNotEmpty) {
  ...
  Future<bool> _cancelaSeparacao() async {
    if (_fase == 0) {
